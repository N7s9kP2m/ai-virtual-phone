...
export const MIX_BUILTIN_VERSION = 6;
...
export function createBuiltinVoiceConsole(): MixMechanismMaterial {
    return {
        ...
        kind: "mechanism",
        name: "官方 · 语音伴侣",
        hook: "后台静默语音朗读机括：作为背景隐形常驻，对白后小喇叭点击即念，自动跟随全局语音设置",
        author: "独家特调",
        tags: ["官方", "语音", "机括", "后台", "朗读"],
        layout: { x: 0, y: 0, w: 100, h: 10, slot: "hidden" },
        panelHtml: [
            ...
            "    var store = window.MIX_STORE || {};",
            "    var url = store.voiceApiUrl || DEFAULT_URL;",
            "    var voice = store.selectedVoice || '';",
            "    var apiKey = store.voiceApiKey || '';",
            "    try {",
            "      var raw = localStorage.getItem('ai_phone_voice_configs_v1');",
            "      if (raw) {",
            "        var list = JSON.parse(raw);",
            "        if (Array.isArray(list) && list.length > 0) {",
            "          var active = list.find(function(c){ return c.enableTTS && (c.baseUrl || c.defaultVoice); }) || list[0];",
            "          if (active) {",
            "            if (!store.voiceApiUrl && active.baseUrl) url = active.baseUrl;",
            "            if (!store.selectedVoice && active.defaultVoice) voice = active.defaultVoice;",
            "            if (!store.voiceApiKey && active.apiKey) apiKey = active.apiKey;",
            "          }",
            "        }",
            "      }",
            "    } catch(e) {}",
            ...
            "        if (!store.voiceApiUrl && url.indexOf('192.168.31.188') !== -1) {",
            "          url = 'http://' + location.hostname + ':8190/v1';",
            "        }",
            "      }",
            "    } catch(e) {}",
            "    return { url: url.replace(/\\/+$/, ''), voice: voice, apiKey: apiKey };",
            "  }",
            "  async function synthAudio(text, cfg) {",
            "    var reqUrl = cfg.url + (cfg.url.endsWith('/v1') ? '/audio/speech' : '/v1/audio/speech');",
            "    if (cfg.url.endsWith('/audio/speech')) reqUrl = cfg.url;",
            "    var headers = { 'Content-Type': 'application/json' };",
            "    if (cfg.apiKey) headers['Authorization'] = 'Bearer ' + cfg.apiKey;",
            "    else headers['Authorization'] = 'Bearer sk-local-none';",
            "    var payload = { model: 'tts-1', input: text, response_format: 'mp3' };",
            "    if (cfg.voice) payload.voice = cfg.voice;",
            "    var resp = await fetch(reqUrl, { method: 'POST', headers: headers, body: JSON.stringify(payload) });",
            "    if (!resp.ok) {",
            ...
            "    var cacheKey = (cfg.voice || 'default') + '::' + text;",
            "    if (cache[cacheKey]) {",
            ...
            "    try {",
            "      var bytes = await synthAudio(text, cfg);",
